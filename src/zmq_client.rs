...
pub struct Router {
    ...
    send_one_reply_channels: HashMap<String, (oneshot::Sender<MessageAndType>, Instant)>,
    // Send all reply channels are used for request/response messages that are broadcast to all
    // connected validators, such as subscriptions. All must respond before the
    // aggregated responses are sent to consumers
    #[allow(clippy::type_complexity)]
    send_all_reply_channels: HashMap<
        String,
        (
            usize,
    ...
    >,
    // Unsolicited channels route messages that are not in response to a
    // request, like ClientEvents.
    unsolicited_channels: HashMap<MessageType, broadcast::Sender<Arc<MessageAndType>>>,
    // Command channel for the communication task
    command_channel: tokio::sync::mpsc::Sender<SocketCommand>,
    // LRU cache of message hashes to prevent duplicate unsolicited messages being broadcast
    lru: LruCache<[u8; 32], ()>,
}
...
impl Router {
    fn new(reply: tokio::sync::mpsc::Sender<SocketCommand>) -> Self {
        Self {
            send_one_reply_channels: HashMap::new(),
            send_all_reply_channels: HashMap::new(),
            unsolicited_channels: HashMap::new(),
    ...
    fn gc_timed_out_channels(&mut self, older_than: Duration) {
        ...
        for (k, (_, t)) in self.send_one_reply_channels.iter() {
        ...
        for k in to_remove {
            ...
            self.send_one_reply_channels.remove(&k);
        }

        let mut to_remove = vec![];
        for (k, (_, _, _, t)) in self.send_all_reply_channels.iter() {
            if (t.elapsed() + older_than) < now.elapsed() {
                to_remove.push(k.clone())
            }
        }
        ...
        for k in to_remove {
            ...
            self.send_all_reply_channels.remove(&k);
        }
    }
    ...
    async fn route(&mut self, connection: usize, message: crate::messages::Message) {
        ...
        if message_type == MessageType::ClientEvents {
            ...
            if self.lru.get(&*message_hash).is_none() {
                ...
                if let Some(broadcast) = self.unsolicited_channels.get(&message_type) {
                    broadcast
        ...
        // Check if this is a send_and_receive_all message
            ...
            self.send_all_reply_channels.remove(&message.tx_id)
        {
            ...
            if responses.len() == required_responses {
                ...
                self.send_all_reply_channels.insert(
                    message.tx_id,
        ...
        // Otherwise this is send_and_receive_one
            ...
            match self.send_one_reply_channels.remove(&message.tx_id) {
